...
import com.diphons.dkmsu.ui.component.CpuClusterStatus;
import com.diphons.dkmsu.ui.component.CpuStatus;
import static com.diphons.dkmsu.ui.util.KsuCliKt.getKernelPropLong;
...
public class CpuFrequencyUtils {
    ...
    private final String cpuinfo_max_freq = cpufreq_sys_dir + "cpuinfo_max_freq";
    ...
    private String getCpuFreqValue(String path) {
        long freqValue = getKernelPropLong(path);
        if (freqValue > -1) {
            return "" + freqValue;
        }
        return "";
    ...
    public String[] getAvailableFrequencies(Integer cluster) {
        ...
        if (new File(scaling_available_freq.replace("cpu0", cpu)).exists()) {
            list_freq = KernelProrp.INSTANCE.getProp(scaling_available_freq.replace("cpu0", cpu));
            if (!list_freq.contains(getInfoMaxFrequency(cluster))) {
                list_freq = list_freq + " " + getInfoMaxFrequency(cluster);
            }
            frequencies = list_freq.split("[ ]+");
            return frequencies;
    ...
    public String getInfoMaxFrequency(Integer cluster) {
        if (cluster >= getClusterInfo().size()) {
            return "";
        }
        String cpu = "cpu" + getClusterInfo().get(cluster)[0];
        return KernelProrp.INSTANCE.getProp(cpuinfo_max_freq.replace("cpu0", cpu));
    }
    ...
    */
    ...
    public ArrayList<String> buildShell(CpuStatus cpuStatus) {
        ...
        if (cpuStatus != null) {
            ...
            // CPU
            if (cpuStatus.cpuClusterStatuses != null && !cpuStatus.cpuClusterStatuses.isEmpty()) {
