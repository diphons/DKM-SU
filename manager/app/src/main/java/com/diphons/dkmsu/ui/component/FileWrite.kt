...
/**
 * Provides public methods to read and write files to external storage
...
object FileWrite {
    ...
    fun getPrivateFilePath(context: Context, outName: String): String {
        return getPrivateFileDir(context) + (if (outName.startsWith("/")) outName.substring(
            1,
            outName.length
        ) else outName)
    }
    ...
    //Convert DOS to Unix to avoid script parsing failure caused by \r\n
